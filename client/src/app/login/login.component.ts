...
export class LoginComponent {
  ...
  updateForm() {
    if (this.mode == 'register') {
      ...
      this.form.addControl('displlay_name', new FormControl(null, [Validators.required, Validators.minLength(3), Validators.maxLength(20)]))
    ...
    switch (ctrlName) {
      case 'username':
        if (control.hasError('required'))
          this.errorMessages.username.set('notnotnotnotnot')
      ...
      case 'password':
        if (control.hasError('required'))
          this.errorMessages.password.set('notnotnotnotnot')
          ...
          this.errorMessages.password.set('must contain mininum of 1 lower-case letter')
